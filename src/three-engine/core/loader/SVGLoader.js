...
SVGLoader.prototype = Object.assign( Object.create( SVGLoader.prototype ), {
    ...
    parse: function ( text ) {
        ...
        function parseNode( node, style ) {
            ...
            if ( path ) {
                ...
                }else if(path instanceof SVGText) {
                    if (style.stroke !== undefined && style.stroke !== 'none') path.color = style.stroke;
                    paths.push(path);
                }
            }
                ...
                switch ( type ) {
                    ...
                    case 'T':
                        ...
                        for ( j = 0, jl = numbers.length; j < jl; j += 2 ) {
                            ...
                            rx = getReflection( point.x, control.x );
                            ry = getReflection( point.y, control.y );
                            path.quadraticCurveTo(
                                rx,
                                ry,
                                numbers[ j + 0 ],
		...
		*/
        ...
        function parseTextNode( node ) {
            var path = new SVGText(node.innerHTML || '');
            path.size = parseFloat(node.getAttribute( 'font-size' ) || 1.5);

            var elements = [];
            var matrix = new THREE.Matrix4();
            var str = node.getAttribute( 'transform' ).replace('matrix(','').replace(')','').split(' ');
            str.forEach(s=>{elements.push(s.indexOf('e') < 0 ? parseFloat(s):0)});
            if(elements.length === 6) {
                matrix.elements = [elements[0],elements[2],0,0,
                    elements[1],elements[3], 0,0,
                    0,0,1,0,
                    elements[4],elements[5],0,1];
            }

            path.mtxLocal = matrix;
            return path;
        }
...
SVGLoader.prototype.getStrokeStyle = function ( width, color, opacity, lineJoin, lineCap, miterLimit ) {
...
SVGLoader.prototype.pointsToStroke = function ( points, style, arcDivisions, minDistance ) {
...
SVGLoader.prototype.pointsToStrokeWithBuffers = function () {
